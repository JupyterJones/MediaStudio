...
def resize_if_needed(clip, target_size=(512, 768)):
    """Resizes the clip if it is not already the target size and ensures it is RGB."""
    if clip.size != target_size:
        logging.info(f"Image size {clip.size} is not {target_size}, resizing...")
        clip = clip.resize(target_size)
    if clip.mask is None:  # Ensure a mask exists
        logging.info("Adding a mask to the clip.")
        clip = clip.add_mask()
    if clip.img.ndim == 2:  # Convert grayscale to RGB
        logging.info("Converting grayscale image to RGB.")
        clip = clip.to_RGB()
    return clip
...
def slide_apart_animation(foreground_path, background_path, output_path, duration=3, fps=24):
    ...
    def make_frame(t):
        ...
        # Get background frame and ensure it is RGB
        bg_frame = background.get_frame(t)
        if bg_frame.ndim == 2:  # Convert grayscale to RGB
            bg_frame = np.stack((bg_frame,) * 3, axis=-1)
        frame[:, :, :] = bg_frame

        # Add the left sliding half
        if slide_distance < 256:
            left_part_width = 256 - slide_distance
            left_frame = left_half.get_frame(t)
            if left_frame.ndim == 2:  # Convert grayscale to RGB
                left_frame = np.stack((left_frame,) * 3, axis=-1)
            frame[:, :left_part_width] = left_frame[:, slide_distance:256]

        # Add the right sliding half
        if slide_distance < 256:
            right_part_width = 256 + slide_distance
            right_frame = right_half.get_frame(t)
            if right_frame.ndim == 2:  # Convert grayscale to RGB
                right_frame = np.stack((right_frame,) * 3, axis=-1)
            frame[:, right_part_width:] = right_frame[:, :256 - slide_distance]
...
def get_image_list():
    image_directory = "static/project/"
    # Get the list of images in the directory and sort them by date last modified
    image_list = sorted(glob.glob(os.path.join(image_directory, '*.jpg')))+ sorted(glob.glob(os.path.join(image_directory, '*.png')))
...
def concatenate_videos(video_paths, output_directory):
    ...
    final_output_directory = "static/vids/"
...
def add_title_image(video_path, hex_color="#A52A2A"):
    ...
    uid = uuid.uuid4().hex
    #if not exist "static/vids/", create it
    if not os.path.exists("static/vids/"):
        os.makedirs("static/vids/")
        print(f"Directory 'static/vids/' created.")
    else:
        print(f"Directory 'static/vids/' already exists.")
    output_path = f'static/temp_exp/OPEN_doorsX.mp4'
    composite_clip.write_videofile(output_path)
    mp4_file = f"static/vids/open_door_ID{uid}.mp4"
