...
import uuid
import shutil
from moviepy.editor import VideoFileClip, ImageClip, ColorClip, CompositeVideoClip, AudioFileClip

# Configure logging
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')

def prep_homedirectory():
    image_directory = 'static/videoresources'
    ...
    for f in os.listdir('static/project'):
        if f.endswith(('.jpg', '.jpeg', '.png')):
            logging.info(f"Copying {f} to {image_directory}")
            shutil.copy(os.path.join('static', 'project', f), image_directory)
...
def convert_to_jpg(directory):
    for file in glob.glob(f"{directory}/*.[jp][pn]g"):
...
def image_dir_to_zoom():
    selected_directory = 'static/videoresources'
    directory = 'static/videoresources'
    ...
    widt, heigh = 1024, 1536

    ffmpeg_cmd = (
        f"ffmpeg -hide_banner -pattern_type glob -framerate {frame_rate} "
        f"-i '{selected_directory}/*.jpg' "
        f"-vf \"scale={widt}x{heigh},zoompan=z='min(zoom+{zoom_increment},1.5)':x='iw/2':y='ih/2':d={zoom_duration}:s={width}x{height}\" "
        f"-c:v libx264 -pix_fmt yuv420p -r {frame_rate} -y {output_video}"
    )
...
def add_title_image(video_path, hex_color="#A52A2A"):
    ...
    output_path = 'static/temp_exp/final_zooms_expX.mp4'
    composite_clip.write_videofile(output_path)
    
    # Save output and speed up the video
    unique_name = f"AI_Creates_Composite_{uuid.uuid4()}.mp4"
    shutil.copyfile(output_path, f"static/vids/{unique_name}")
    return output_path
...
if __name__ == "__main__":
    ...
    if video_path:
        ...
        logging.info(f"Final video created at: {final_video}")