#!/home/jack/miniconda3/envs/PY39/bin/python3
# -*- coding: utf-8 -*-
import base64
...
for d in [COMPUTER_IMAGE_DIRECTORY, STATIC_DIR, UPLOAD_DIR, OVERLAY_DIR, RESULT_DIR]:
...
def refresh_images():
    try:
        ...
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "refresh_images.py"],
            check=True,
        )
        return redirect(url_for("index"))
...
def refresh_video():
    try:
        ...
        createvideo()
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "refresh_video.py"],
            check=True,
        )
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "Best_FlipBook"], check=True
        )
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "diagonal_transition"],
            check=True,
        )
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "blendem"], check=True
        )
        subprocess.run(["/bin/bash", "slide"], check=True)
        subprocess.run(["/bin/bash", "zoomX4"], check=True)
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "fadem"], check=True
        )
        subprocess.run(["/bin/bash", "zoomY4"], check=True)
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "vertical_scroll"],
            check=True,
        )
        ...
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "joinvid"], check=True
        )
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            background_image_path = random.choice(
                glob.glob("static/archived-images/*.jpg")
            )
...
def remove_images():
    junk_folder = "static/junk_resources/"
    # if not os.path.exists(junk_folder):
    if not os.path.exists(junk_folder):
...
def vertical_scroll_route():
    try:
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "vertical_scroll.py"],
            check=True,
        )
        return redirect(url_for("mk_videos"))
    except subprocess.CalledProcessError as e:
...
def join_video_route():
    try:
        subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", "join_all_videos"],
            check=True,
        )
        return redirect(url_for("mk_videos"))
    except subprocess.CalledProcessError as e:
...
def resize_mp4_route():
    ...
    output_file = (
        "/home/jack/Desktop/Flask_Make_Art/static/temp_exp/all_asset_videos.mp4"
    )
...
@app.route("/fadem")
def fadem():
    subprocess.run(["/home/jack/miniconda3/envs/PY39/bin/python", "fadem"], check=True)
    return redirect(url_for("mk_videos"))
...
def start_tensorflow_server():
    ...
    try:
        ...
        server_script = (
            "python -m http.server 8000 --directory /mnt/HDD500/TENSORFLOW/models/"
        )

        # Start the server as a subprocess
        print("Starting TensorFlow server...")
        process = subprocess.Popen(
            ["bash", server_script], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def open_doors_route():
    subprocess.run(
        ["/home/jack/miniconda3/envs/PY39/bin/python", "open_door_directory.py"],
        check=True,
    )
    return redirect(url_for("mk_videos"))
...
def square_transition_route():
    subprocess.run(
        ["/home/jack/miniconda3/envs/PY39/bin/python", "square_transition.py"],
        check=True,
    )
    return redirect(url_for("mk_videos"))
...
def circular_transition_route():
    subprocess.run(
        ["/home/jack/miniconda3/envs/PY39/bin/python", "circular_transition.py"],
        check=True,
    )
    return redirect(url_for("mk_videos"))
...
def mk_avatar_route():
    ...
    if image_file and audio_file:
        ...
        dest1 = "/home/jack/Desktop/MediaStudio/static/Wav2Lip-master/sample_data/use.jpg"
        dest2 = "/home/jack/Desktop/MediaStudio/static/Wav2Lip-master/sample_data/use.mp3"
...
def makeit():
    subprocess.run(
        ["/home/jack/miniconda3/envs/PY39/bin/python", "Wav2Lip-master/makeit"],
        check=True,
    )
    return redirect(url_for("mk_videos"))
...
def create_search_database():
    ic("Starting create_search_database()")
    ...
    if not os.path.exists(history_file):
        ic("No bash history file found")
        conn.close()
        return

    # Read entire history and keep ONLY the last 20 lines
    with open(history_file, "r", encoding="utf-8", errors="ignore") as file:
        lines = file.readlines()[:20]

    ic("Total lines loaded:", len(lines))
    ...
    for line in lines:
        ...
        try:
            ...
            ic("Duplicate ignored:", command)

    conn.commit()
    conn.close()

    ic(f"Database update complete. Inserted {inserted} commands.")
...
def create_search_database():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS history (command TEXT PRIMARY KEY)"
    )  # Set command as PRIMARY KEY
    ...
    if os.path.exists(history_file):
        with open(history_file, "r", encoding="utf-8", errors="ignore") as file:
                ...
                if command:  # Check if the line is not empty
                    try:
                        cursor.execute(
                            # demo only use command[:10] first ten lines command 
                            # command without brackets lists all bash history
                            "INSERT INTO history (command) VALUES (?)", (command,)
                        )
                    except sqlite3.IntegrityError:
...
def search_history():
    ...
    if request.method == "POST" and "query" in request.form:
        ...
        with get_db_conn() as conn:
            cursor = conn.execute(
                "SELECT command FROM history WHERE command LIKE ?", ("%" + query + "%",)
            )
            history_results = (
                cursor.fetchall()
            )  # Fetch all matching results from the database
    ...
    if "temp_query" in request.form:
        ...
        if os.path.exists("tempfile.txt"):
            with open("tempfile.txt", "r") as temp_file:
                temp_results = [
                    line.strip() for line in temp_file if temp_query in line
                ]  # Search for temp_query in tempfile.txt
    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "/home/jack/Desktop/Flask_Make_Art/Wav2Lip-master/makeavatar",
            ],
            check=True,
        )
...
def add_title(video_path, hex_color="#A52A2A"):
    ...
    composite_clip.write_videofile(output_path)
    shutil.copyfile(
        output_path, f"/mnt/HDD500/collections/vids/square_mask_transitionX_{uid}.mp4"
    )
...
def add_shrink_title_image(video_path):
    ...
    composite_clip.write_videofile(output_path)
    unique_output_path = (
        f"/mnt/HDD500/collections/vids/AI_Flipbook_{uuid.uuid4().hex}.mp4"
    )
...
def add_zoom_title_image(video_path, hex_color="#A52A2A"):
    ...
    mp3_files = glob.glob("static/mp3/*.mp3")
    random.shuffle(mp3_files)
    music_clip = (
        AudioFileClip(random.choice(mp3_files))
        .audio_fadein(0.5)
    ...
    shutil.copyfile(
        output_path, f"/home/jack/Desktop/HDD500/collections/vids/{unique_name}"
    )
...
def gc_plot():
    ...
    if len(gc_data) > 1:
        ...
        ax.set(
            ...
            ylabel="Number of Objects Collected",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            result = subprocess.run(
                ["/home/jack/miniconda3/envs/PY39/bin/python", script_path],
                capture_output=True,
                text=True,
                check=True,
            )
...
BALACOON_MODEL_PATH = (
    "/home/jack/hidden/en_us_cmuartic_jets_cpu.addon"  # Ensure this path is correct
)
tts_instance = None
...
for p in [UPLOADS, CLIPS, AUDIO, OUTPUT, THUMBS]:
...
def make_avatar():
    try:
        # /home/jack/Desktop/effects/make_avatar.py
        script_path = "/home/jack/Desktop/MediaStudio/static/Wav2Lip-master/makeavatar"
        result = subprocess.run(
            ["/home/jack/miniconda3/envs/PY39/bin/python", script_path],
        ...
        mp4_file = (
            "/home/jack/Desktop/MediaStudio/static/temp_exp/avatarX.mp4"
        )
...
def convert():
    ...
    {% with messages = get_flashed_messages() %}
      {% if messages %}
        <ul class=flashes style="color: white; background-color: black; padding: 10px;">
      ...
      {% endif %}
    {% endwith %}
    <h1 style="color: #00ffff;">🎙️ Upload Audio (e.g., AAC, M4A, WAV, MP4 audio track) to Convert to MP3</h1>
    <form method=post enctype=multipart/form-data>
...
def download_page():
    ...
    <body style="background-color:darkred;" >
    <h2 style="color: #00ff99;">🎧 Your MP3 file is ready: {os.path.basename(file_path_in_static_relative)}</h2>
    <audio controls>
    <source src="{{{{ url_for('static', filename='{file_path_in_static_relative}') }}}}" type="audio/mpeg">
    Your browser does not support the audio element.
    </audio>
    <br><br>
    <a href="{{{{ url_for('static', filename='{file_path_in_static_relative}') }}}}" download>⬇️ Download MP3</a>
    <br><br>
    <a href="{{{{ url_for('convert') }}}}">🔁 Convert Another</a>
    </body>
    """
    )
    ...
    if request.method == "POST":
        ...
        if "delete_images" in request.form:
            ...
            if selected_static_path:
                ...
                for image in images_to_delete:
                    try:
                        os.remove(os.path.join(disk_path, image))
                        ic(f"Deleted {image}")
                    except OSError as e:
