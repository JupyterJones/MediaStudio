...
<head>
  ...
  <style>
    ...
    .thumb label { cursor: pointer; display: flex; align-items: center; flex-direction: column; text-align: center; }
    ...
    .note { color: #888; font-size: 90%; word-break: break-all; }
    .nav-link {
...
<div class="container">
  ...
  <a href ="/form_page" class="nav-link">Create Video</a>
  ...
  <p class="note">Sorted newest → oldest. Assign a number (1-{{ MAX_SELECT }}) to exactly {{ MAX_SELECT }} images to set their order in the video. Then choose one MP3 and click Create Video.</p>

  <form action="{{ url_for('create_video') }}" method="post">
    <div class="section">
      <h3>Images (set order for {{ MAX_SELECT }})</h3>
      <div class="image-grid">
      {% if images %}
        {% for img in images %}
          <div class="thumb">
            <label for="order_{{ loop.index0 }}">
              <img src="static/projects/{{ img }}" alt="{{ img }}">
              <div>
                <input type="number" name="{{ img }}" id="order_{{ loop.index0 }}" min="1" max="{{ MAX_SELECT }}" style="width: 50px; text-align: center; margin-top: 5px;" placeholder="#">
                <div style="margin-top: 5px;"><b>{{ img }}</b></div>
    ...
    <div class="controls">
      <button type="submit">Create Video from selected images + mp3</button>
    </div>
  </form>
...
def create_video():
    all_image_files = set(list_media()["images"])
    ordered_images = []
    for filename, order_str in request.form.items():
        if filename in all_image_files and order_str:
            try:
                order = int(order_str)
                ordered_images.append((order, filename))
    ...
    # Validation
    if len(ordered_images) != MAX_SELECT:
        msg = f"ERROR: You must assign an order number to exactly {MAX_SELECT} images. You assigned numbers to {len(ordered_images)}."
        logs.append(msg)
        ic(msg)
        return render_gallery_with_message("\n".join(logs))

    orders = [item[0] for item in ordered_images]
    if len(orders) != len(set(orders)):
        msg = f"ERROR: You have used duplicate order numbers. Please use each number from 1 to {MAX_SELECT} only once."
        logs.append(msg)
        ic(msg)
        return render_gallery_with_message("\n".join(logs))

    expected_orders = set(range(1, MAX_SELECT + 1))
    if set(orders) != expected_orders:
        msg = f"ERROR: You must use the numbers from 1 to {MAX_SELECT}. You used: {sorted(orders)}"
        logs.append(msg)
        ic(msg)
        return render_gallery_with_message("\n".join(logs))

    # Sort images by the specified order and get the filenames
    ordered_images.sort(key=lambda x: x[0])
    selected_images = [item[1] for item in ordered_images]
    
    ic(f"Selected images in order: {selected_images}")
    ...
    # create slides.txt with relative paths
    ...
    try:
        with open(slides_txt, "w", encoding="utf-8") as fh:
            for img in sel_images:
                # Use relative paths (just the filename)
                fh.write(f"file '{img}'\n")
                fh.write(f"duration {per_image}\n")
            # repeat last
            # Use relative path for the last image as well
            fh.write(f"file '{sel_images[-1]}'\n")
    ...
    temp_video_basename = f"slideshow_{uuid.uuid4().hex}.mp4"
    temp_video = os.path.join(UPLOADS_DIR, temp_video_basename)
    slides_txt_basename = os.path.basename(slides_txt)
    create_slideshow_cmd = ["ffmpeg", "-hide_banner", "-y", "-f", "concat", "-safe", "0", "-i", slides_txt_basename,
                            "-vsync", "vfr", "-pix_fmt", "yuv420p", temp_video_basename]
    try:
        run_cmd(create_slideshow_cmd, cwd=PROJECT_DIR)
        sleep(1)
...
def join_audio_video():
    ...
    sleep(1)
    unique_id = str(uuid4())
    final_output = os.path.join(UPLOADS_DIR, f"final_joined_{unique_id}.mp4")
...
<a href ="/dream" class="nav-link">Dream</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href ="/" class="nav-link">HOME</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/create_text">Create Mp3</a>&nbsp;&nbsp|;&nbsp;&nbsp;<a href="/view_text">View Text</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href ="/form_page" class="nav-link">Create Video</a>
...
def enhance():
    ...
    <body>
        ...
        <a href="/create_text">Create Mp3</a>&nbsp;&nbsp|;&nbsp;&nbsp;<a href="/view_text">View Text</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href ="/form_page" class="nav-link">Create Video</a>
...
def sanitize_for_tts_filename(text, max_length=20):
    base = text.strip()[:max_length]
    base = re.sub(r'\W+', '_', base)
    return base or "output"

def generate_tts(text, voice):
    filename = sanitize_for_tts_filename(text) + ".mp3"
    output_path = os.path.join(OUTPUT_DIR, filename)
...
@app.route("/publish")
def publish_video():
    ic("Starting publish route")

    intro = "static/projects/intro.mp4"
    main = "static/projects/main.mp4"

    unique_id = str(uuid.uuid4())
    output = f"static/projects/{unique_id}.final.mp4"
    concat_list = "concat_list.txt"

    ic(intro, main, output)
    ...
    ic("Running ffmpeg:", cmd)

    try:
        subprocess.run(cmd, check=True)
        ic("FFmpeg completed successfully")
    except Exception as e:
        ic("Error during ffmpeg:", e)
        return f"Error: {e}"

    # Cleanup concat list
    try:
        os.remove(concat_list)
        ic("concat_list.txt removed")
    except:
        ic("Could not remove concat_list.txt")

    return send_from_directory("static/projects", f"{unique_id}.final.mp4", as_attachment=True)
# -------------------------------------------------
# MAIN
# -------------------------------------------------
