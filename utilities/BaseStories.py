...
FINAL_STORY_FILE = os.path.join(OUTPUT_DIR, f"final_{STORY}_auto.txt")
'''
FIRST_BULLET_PRIMER = (
...
'''
FIRST_BULLET_PRIMER = (
    "When working with AI training, the model must be given rules and goals. A model may not injure a human without just cause, a model must obey the orders given it by human beings. A model must protect its own existence. A group is more important than one."
)
OUTLINE_BULLETS = [
